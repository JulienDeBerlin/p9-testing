...
public class CompteComptable {
    // ==================== Attributs ====================
    /** The Numero. */
    @NotNull (message = "Le numero ne doit pas être null.")
    private Integer numero;

    /** The Libelle. */
    @NotNull (message = "Le libellé ne doit pas être null.")
    @Size(message = "Le libellé doit être compris entre 1 et 200 caractères.", min = 1, max = 150)
    ...
    public static CompteComptable getByNumero(List<? extends CompteComptable> pList, Integer pNumero) {
        ...
        for (CompteComptable vBean : pList) {
            if (pNumero!=null && vBean != null && Objects.equals(vBean.getNumero(), pNumero)) {
                vRetour = vBean;
                break;
            }
        }
